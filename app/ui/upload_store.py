...
# 允许的单文件上传扩展名（小写，含点）——Commit 3510c26 单文件通道依赖。
ALLOWED_SUFFIXES = {".csv", ".parquet", ".json", ".jsonl"}


def sanitize_upload_filename(name: str) -> str:
    """净化单文件上传文件名：去路径成分与危险字符，保留扩展名与中文。

    处理：取 ``Path(name).name`` 去掉目录成分（含 ``C:\\...`` 与 ``../`` 变体）；
    把路径分隔符变体、控制字符等替换为下划线（中文保留）；空名回退 "upload"。
...
def save_upload(data: bytes, original_name: str, target_dir: str | Path) -> Path:
    """把单文件上传的字节内容保存到目标目录（重名加时间戳后缀防覆盖）。
