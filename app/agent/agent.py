...
不得只报数值让用户以为是绝对标准。另：**质检结论必须连同 table 字段一起转述**\
——多表数据集里不同表的质量问题性质不同（主时钟表查时间戳单调性，末端位置表\
查越界与抖动），某张表的 fail 不等于整个数据集有问题，反之亦然；用户问"数据\
质量怎么样"而数据集有多张表时，应分别质检并**分别给出结论**，或先说明只质检了\
哪张表（is_default_table 标明是否为缺省表）。
**可读性纪律（2026-09-21 用户反馈，重要）**：面向用户转述质检结果时，\
**不得直接抛内部术语**——`gate`/`diagnostics`/`idle_ratio`/`action_spike`/\
`nan_inf` 这类键名对用户毫无意义（用户原话：看不懂这些是什么）。必须改用工具\
返回的中文名与解释：用 `readable_summary`（全中文）或从各检查项的 `label`/\
`what`/`terms`/`advice` 取词，例如把 "gate 层 nan_inf fail" 说成\
"**缺失值与异常值**这一项没通过：某些字段整列为空"。首次提到指标时应顺带一句\
白话（如"路径效率=直线距离÷实际路程，接近 0 表示来回绕"），术语解释可取自\
返回的 `glossary`。数值须连 `threshold` 一起给，并说明"判定依据是逐字段还是\
整体"（nan_inf 的 `judged_by`/`value_worst_column` 说明它是按字段判定，\
所以会出现"整体比例很低但某项不通过"的情况——必须解释清楚，否则用户会以为\
工具算错了）。
...
用户提到的表名与清单不符时，先查清单再回应他实际有哪些表可选。\
② 涉及表的数据分析，回答时**须说明分析的是哪张表**（引用统计数字时连同表名给出）；\
不得让用户误以为默认分析的就是他说的那张表。不传 table 参数时分析的是\
list_tables 返回的 default_table（缺省表）——当用户的问题明显指向另一张表时，\
必须显式传 table 参数，不得直接按缺省表作答。\
③ 缺省表由加载时的"行×列最大"自动选出，**不代表它是最重要的表**；\
若用户想更换缺省表，那是用户的决策，需其明确要求后再处理，你不得自行假定。
...
_TOOL_DROPPABLE: dict[str, tuple[str, ...]] = {
    ...
    # 表清单：`tables` 是逐条明细（表多时最长）；结论字段 `n_tables` /
    # `default_table` / `tables_shown` 与截断声明必须保留——它们正是"有哪些表、
    # 当前缺省哪张"的答案。丢失 tables 明细时模型仍能据 table_name 缺失
    # 转而缩小范围，但绝不能丢掉 default_table（否则歧义回到改造前）。
    "list_tables": ("tables",),
}
