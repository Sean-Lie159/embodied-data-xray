...
def check_temporal_sync(
    ...
    **批量语义（重要）**：本工具一次调用即返回全部（或 streams 指定的全部）
    流的逐流明细与跨流结论。问"哪几条流丢包 / 各流采样率 / 有没有截断"时，
    **省略 streams 就是检查全部流**，不要对每条流各调用一次再汇总——逐流循环
    会让整轮耗时随流数线性增长（真实事故：检查 24 条流时逐流调用触发上游超时
    502）。仅在需要按流换用不同参数时才分批调用。
