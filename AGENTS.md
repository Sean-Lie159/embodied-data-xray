...
8. **临时脚本的生成与清理**（违反即污染正式文件）：
   - AI 为"追加/修改文档"而写的临时脚本，**禁止把脚本头写进目标文档**——
     必须用独立的 `.py` 文件，且**用完立即删除**；
   - 临时脚本统一放 `scripts/` 下并**以 `_` 前缀命名**（如 `scripts/_probe.py`），
     便于识别与定期清理；
   - **提交前必须 `git status` 确认无临时脚本残留**（`git add -A` 会把它们一起加入）；
   - **历史事故（2026-09-22）**：一段"追加文档"的临时脚本被误写进
     `docs/行为测试.md`，脚本头 + 未闭合的 `anchor = "` 把文件开头 **31 行**
     包成了 Python 字符串字面量，文档结构被破坏而长期无人察觉。
     修复见该文件开头的说明注记。
   - **AI 自查义务**：每轮任务收尾时应主动清理自己创建的临时文件；
     若发现**他人遗留**的临时文件，**指出并询问**，不要擅自删除。
