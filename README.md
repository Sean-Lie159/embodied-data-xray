...
把**你自己的一台电脑**变成团队数据站：同事用浏览器访问 `http://<你的内网IP>:8501`，
只做分析和看图，模型配置对他们不可见。

1. **部署机配置**：在部署机 `.env` 完成模型配置（密钥、接口地址、模型名），
   另加两行可选配置：
   ```ini
   INSTANCE_MODE=1          # 开启后访问者看不到"模型设置"面板
   ACCESS_PASSWORD=你的口令  # 可选：简单门禁（内网明文，仅限可信网络）
   ```
2. **启动**：双击 `scripts/serve_lan.bat`（或手动
   `streamlit run streamlit_app.py --server.address 0.0.0.0 --server.port 8501 --server.headless true`）；
3. **放行防火墙**：Windows 防火墙放行 TCP 8501（首次启动会弹窗，选"允许"；
   或手动添加入站规则）；
4. **同事访问**：`http://<你的内网IP>:8501`，输口令（如启用）后即进入；
   数据通过侧栏「上传压缩包数据集（.zip）」或「上传单个数据文件」进入部署机
   （Agent 只能读部署机的文件系统，这是设计使然）。

安全边界：设计定位是**可信内网的简单共享**——Streamlit 无 HTTPS（口令与数据
明文传输），无多用户隔离（大家共享 `outputs/`），不要把该服务直接暴露到公网；
如需公网演示，只建议用「示例数据集」功能。
