...
| 层 | 包含 | 能判 fail？ | 怎么理解 |
|---|---|---|---|
| 硬门禁 gate | NaN/Inf、时间戳非单调、丢帧、跨 episode schema 不一致、fps 非法、episode 边界矛盾 | ✅ 只有这层能 | 确定性错误，出现即为真问题 |
| 诊断项 diagnostics | 空闲比、动作突波、致动器饱和、动作抖动、长停顿、异常振动、路径效率、视频元信息 | ❌ 只提示 | 启发式指标，**超阈不等于数据有问题** |

分层是刻意的：**阈值与任务强相关**，不存在通用阈值——70% 空闲比对 push/放置类
任务可能完全正常，对 lift 类才可疑。若让诊断项自动升级为判定，会产生大面积误报
（同类工具曾因此误报 65%）。所以诊断项的 `warn` 只是提示，需结合任务类型判断。
